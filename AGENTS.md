...
| `repair-dk-package-github-actions` | `agents/repair-dk-package-github-actions.agent.md` | Diagnosing or repairing a GitHub Actions-based dk package workflow. | Must run `analyze-dk-package-github-actions` first, then ask where to apply downloaded patches if a checkout path is not already provided. Stop if workflow, dk-project, release-prefix, or producer-shaping facts are missing. |
